...
import { API_ENDPOINT } from '@/app/api/urls';
import { 
  SignupSchemaType, 
  LoginSchemaType,
...
export function useSignup() {
    ...
    return useMutation({
      mutationFn: async (data: Omit<SignupSchemaType, 'confirmPassword'> & { role: 'trockler' }) => {
        ...
        const response = await apiClient.post<AuthResponse>(API_ENDPOINT.auth.signup, payload);
...
export function useLogin() {
    ...
    return useMutation({
      mutationFn: async (data: LoginSchemaType) => {
        const response = await apiClient.post<AuthResponse>(API_ENDPOINT.auth.login, data);
        return response.data;
      },
    ...
    return useMutation({
        mutationFn: async (data: { email: string; otp: string }) => {
         const response = await apiClient.post<VerifyEmailResponse>(
            API_ENDPOINT.auth.email.verify,
            data
         );
         return response.data;
        },
    ...
    return useMutation({
        mutationFn: async (data: { email: string }) => {
          const response = await apiClient.post<ForgotPasswordResponse>(
           API_ENDPOINT.auth.password.forgot,
           data
          );
          return response.data;
        },
    ...
    return useMutation({
        mutationFn: async (data: { email: string; otp: string }) => {
          const response = await apiClient.post<VerifyOtpResponse>(
            API_ENDPOINT.auth.password.verifyOtp,
            data
          );
          return response.data;
        },
    ...
    return useMutation({
      mutationFn: async (data: { email: string; newPassword: string; otp: string }) => {
        const response = await apiClient.post<ResetPasswordResponse>(
            API_ENDPOINT.auth.password.reset,
            data
        );
        return response.data;
      },
      ...
      onError: (error: Error) => {
        showErrorToast(error.message || 'Failed to rest password.');
      },
    });
}
